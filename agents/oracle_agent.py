...
class OracleAgent(MatrixAgent):
    ...
    def __init__(self, name: str = "oracle-primary", bus_url: str = "tcp://127.0.0.1:5555"):
    ...
    async def execute(self, task: TaskDefinition) -> Dict[str, Any]:
        ...
        # Dispatch QA / Critic operation via ZMQ hook
        logger.info(f"[{self.name}] Dispatching task execution via ZMQ hooks...")
        try:
            req_id = await self.execute_tool("execute_guillotine_qa", {
                "task_id": task.task_id,
                "instructions": task.instructions,
                "input_data": task.input_data
            })
            return {"status": "dispatched", "agent": self.name, "request_id": req_id, "message": "QA Guillotine assessment dispatched."}
